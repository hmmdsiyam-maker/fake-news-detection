...
interface AdminModalProps {
  ...
  onSearchPayments?: (query: string, status?: string, plan?: string) => void;
...
export const AdminModal: React.FC<AdminModalProps> = ({
  ...
  const [tab, setTab] = useState<"stats" | "users" | "logs" | "blogs" | "payments">("stats");
  ...
  // Payment Filter & Pagination State
  ...
  const filteredPayments = payments.filter((p) => {
    const matchesSearch =
      ...
      (p.stripe_session_id && p.stripe_session_id.toLowerCase().includes(paymentSearch.toLowerCase())) ||
      `#pay-${p.id}`.includes(paymentSearch.toLowerCase());
    const matchesStatus =
      paymentStatusFilter === "all" || p.status.toLowerCase() === paymentStatusFilter.toLowerCase();
    const matchesPlan =
      paymentPlanFilter === "all" || p.plan_id.toLowerCase() === paymentPlanFilter.toLowerCase();
  ...
  const totalRevenueDollars = payments.reduce(
    (acc, p) => (p.status.toLowerCase() === "completed" ? acc + Number(p.amount || 0) : acc),
    0
  );
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-4">
          ...
          {tab === "stats" && stats && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60">
                <div className="text-xs text-slate-500 dark:text-slate-400">Total Payments</div>
                <div className="text-2xl font-bold font-mono mt-1 text-indigo-600 dark:text-indigo-400">
          ...
          {tab === "payments" && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 shadow-2xs">
                  ...
                  <div className="text-[10px] text-slate-400 mt-0.5">All-time collected</div>
                </div>
                ...
                <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 shadow-2xs">
                  ...
                  <div className="text-[10px] text-slate-400 mt-0.5">Total processed</div>
                </div>
                ...
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setPaymentPage(1);
                      if (onSearchPayments) onSearchPayments(e.target.value, paymentStatusFilter, paymentPlanFilter);
                    }}
                    className="w-full pl-9 pr-3 py-1.5 text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"
                  />
                </div>
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setPaymentPage(1);
                      if (onSearchPayments) onSearchPayments(paymentSearch, e.target.value, paymentPlanFilter);
                    }}
                    className="px-2.5 py-1.5 text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setPaymentPage(1);
                      if (onSearchPayments) onSearchPayments(paymentSearch, paymentStatusFilter, e.target.value);
                    }}
                    className="px-2.5 py-1.5 text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer"
                  >
                        ...
                        return (
                          <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                            <td className="p-3">
                              ...
                              {p.stripe_session_id && (
                                <div className="text-[10px] text-slate-400 font-mono truncate max-w-[120px]" title={p.stripe_session_id}>
                                  {p.stripe_session_id}
                                </div>
                              )}
                            </td>
                            <td className="p-3">
                              <div className="font-medium text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
                                <span className="w-5 h-5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 flex items-center justify-center text-[10px] font-bold uppercase">
                                  {(p.username || "U")[0]}
                                </span>
                                <span>{p.username || `User #${p.user_id}`}</span>
                              </div>
                            ...
                            <td className="p-3">
                              <div className="flex items-center gap-1 text-[11px] text-slate-600 dark:text-slate-400 capitalize">
                            ...
                            <td className="p-3">
                              ...
                              >
                                ...
                                {isRefunded && <RotateCcw className="w-2.5 h-2.5" />}
                                <span>{p.status}</span>
                              </span>
                            </td>
                            <td className="p-3 text-right">
